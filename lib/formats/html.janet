...
(defn- render-h [b node]
  ...
  (case (get node :level)
    ...
    (do
      ...
      (set section v)
      (set subsection nil)
      (buffer/push b `<h2 class="section" id="`)
      (buffer-attr b (section-id v))
      (buffer-line b `">` v `</h2>`))
    2
    (do
      (set subsection v)
      (buffer/push b `<h3 class="subsection" id="`)
      (buffer-attr b (section-id v))
      (buffer-line b `">` v `</h3>`))))
...
(defn- render-xref [b node]
  ...
  (case (get node :kind)
    ...
    :section
    (do
      (buffer/push b `<a href="#`)
      (buffer-attr b (section-id (get v 0)))
      (buffer/push b `">` (get v 0) `</a>`))))
...
(defn- doc-frontmatter [root]
  (def node (first root))
  (if (= :frontmatter (get node :type))
    (get node :value)))

(defn- doc-title [root]
  (-?> (doc-frontmatter root) (get :title)))
...
# The prologue is written before any declaration or element so that the
# licence, if there is one, is the first thing in the file.
(defn- render-prologue [b root &named no-ad?]
  (unless no-ad?
    (add-note b))
  (when (def fm (doc-frontmatter root))
    (add-licence b fm)))

(defn- render-body [b program root]
  (set progname program)
  (buffer-line b `<div class="manpage">`)
  (def fm (doc-frontmatter root))
  (when fm
...
(defn render-doc [program root &named no-ad?]
  (def b @"")
  (render-prologue b root :no-ad? no-ad?)
  (render-body b program root)
  (string b))
